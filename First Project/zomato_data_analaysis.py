...
class Customer:
    ...
    def get_error_info(self, e):
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        error_info = (
            ("Error", str(e)), ("ErrorLineNo", str(exc_tb.tb_lineno)), ("ErrorType", str(exc_type)),
            ("ErrorFileName", fname))
        return error_info

    def customer_check(self, customer_id, name, email, phone):
        where = ''
        with self.db.cursor() as curer:
            ...
            # print(query)
            curer.execute( f"SELECT * FROM customers WHERE is_active <> 'delete' and name = '{name}' AND (email = '{email}' OR phone = '{phone}') {where}")
    ...
    def customer_update(self, customer_id, name, email, phone, location, is_premium, preferred_cuisine, average_rating):
        ...
        try:
            with self.db.cursor() as curer:
                if self.customer_check(customer_id, name, email, phone):
                    return "This Data Already Available"
            ...
            return self.get_error_info(str(e))
    ...
    def customer_status_change(self, customer_id, status):
        with self.db.cursor() as curer:
            ...
            curer.close()
            self.db.close()
            return "Customer Status Change Successfully"
...
class Delivery_persons:
    ...
    def persons_update(self, person_id, name, contact_number, vehicle_type, location, total_deliveries, average_rating):
        with self.db.cursor() as curer:
            if self.persons_check(curer, person_id, name, contact_number, vehicle_type):
                return "Already Exsit"
